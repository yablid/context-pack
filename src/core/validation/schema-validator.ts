...
import { SCHEMA_REGISTRY, type SchemaId } from './zod-schemas.js';
...
export class SchemaValidator {
  ...
  /**
   * Validate data directly against a schema ID
   */
  validate(schemaId: string, data: unknown): { success: boolean; errors?: any } {
    const schema = this.schemas.get(schemaId);
    if (!schema) {
      return { success: false, errors: [`Unknown schema ID: ${schemaId}`] };
    }
